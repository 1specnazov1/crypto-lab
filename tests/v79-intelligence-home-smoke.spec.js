...
const TICKERS=BASES.map((base,index)=>({
  symbol:base+'USDT',
  lastPrice:String(PRICES[index]),
  priceChangePercent:String(CHANGES[index]),
  quoteVolume:String(2500000000-index*125000000)
}));
  ...
  await page.route('https://**/*',async route=>{
    ...
    if(url.includes('/functions/v1/crypto-lab-v79-preview')){
      return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({
        ...
        latest_run:{finished_at:new Date().toISOString(),class_a_found:3,class_a:[],symbols_checked:20,timeframes:['5M','1H','4H']},
        signal_counts:{waiting:0,active:0,closed:0},
        scanner_job:{active:true},monitor_job:{active:false},signals:[],runs:[]
      })});
    }
    if(url.includes('/functions/v1/crypto-lab-v79-news')){
      return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,breaking:[]})});
    }
    ...
    if(url.includes('/functions/v1/crypto-lab-v79-recover')){
      return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,enabled:false,site_key:null,captcha_provider:'turnstile',captcha_action:'crypto_recover',email_enumeration_safe:true,readiness:{feature_flag:false,turnstile:true,mail_provider:true}})});
    }
    if(url.includes('data-api.binance.vision')||url.includes('api.binance.com')){
      if(url.includes('/api/v3/ticker/24hr')){
        const multi=url.includes('symbols=');
        return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(multi?TICKERS:TICKERS[0])});
      }
      if(url.includes('/api/v3/ticker/price')){
        return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify(TICKERS.map(x=>({symbol:x.symbol,price:x.lastPrice})))});
      }
      if(url.includes('/api/v3/klines')){
        const now=Date.now();
        const rows=Array.from({length:120},(_,i)=>{const o=64000+i*8,c=o+(i%2?-3:5),t=now-(120-i)*60000;return[t,String(o),String(Math.max(o,c)+12),String(Math.min(o,c)-10),String(c),String(10+i/10),t+59999]});
...
async function open(page){
  await stub(page);
  await page.goto('/v79/app.html?intelligence-home-smoke=1',{waitUntil:'domcontentloaded'});
  await expect(page.locator('#homeView')).toHaveClass(/ih/);
...
async function noOverflow(page){
  ...
  expect(Math.max(d.body,d.doc,d.home)).toBeLessThanOrEqual(d.viewport+3);
}
...
test('intelligence home actions open the existing analytical modules',async({page})=>{
  await open(page);
...
test('intelligence home follows RU UA EN selection and persists it',async({page})=>{
  await open(page);
  await page.locator('#lang').selectOption('uk');
  await expect(page.locator('#title')).toContainText('Ринок за 30 секунд');
  await page.reload({waitUntil:'domcontentloaded'});
  await expect(page.locator('#lang')).toHaveValue('uk');
  await expect(page.locator('#title')).toContainText('Ринок за 30 секунд');
  await page.locator('#lang').selectOption('en');
  await expect(page.locator('#title')).toContainText('Market in 30 seconds');
  await noOverflow(page);
});
