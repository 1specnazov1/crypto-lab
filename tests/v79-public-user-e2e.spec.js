...
const ACCOUNT={effective_plan:'FREE',profile:{display_name:'New User',language:'en',timezone:'UTC',role:'user'},subscription:{status:'active',current_period_end:null},limits:{daily_ai_requests:5,daily_chart_views:100,daily_backtests:50,daily_exact_backtests:25,daily_scanner_views:-1,max_portfolio_assets:-1,max_favorites:-1},usage_today:{ai_requests:0,chart_views:0,backtests:0,exact_backtests:0,scanner_views:0},counts:{portfolio_assets:0,favorites:0}};
...
async function stub(page){
 await page.addInitScript(()=>{
   localStorage.setItem('cryptoLabLanguage','en');
 ...
 await page.route('https://txhzxbizjpinowepfjkm.supabase.co/functions/v1/crypto-lab-v79-register**',async r=>{
  if(r.request().method()==='GET')return r.fulfill({status:200,headers:CORS,contentType:'application/json',body:JSON.stringify({ok:true,enabled:true,registration_mode:'x_follower_free',follower_verification_required:true,target_handle:'@CryptoLabPulse',site_key:'test',captcha_action:'crypto_register',required_legal_keys:['terms','privacy','risk'],documents:DOCS})});
  ...
  if(payload.email!=='new-user@example.invalid')return r.fulfill({status:400,headers:CORS,contentType:'application/json',body:JSON.stringify({ok:false,error:'INVALID_EMAIL'})});
  ...
  return r.fulfill({status:202,headers:CORS,contentType:'application/json',body:JSON.stringify({ok:true,status:'confirmation_pending',follower_verification_required:true,target_handle:'@CryptoLabPulse'})});
 });
...
test('new FREE user can self-register, confirm-flow, login, recover and relogin before follower entitlement',async({page})=>{
 ...
 await expect(page.locator('#followerSignupNote')).toContainText('@CryptoLabPulse');
 ...
 await expect(page.locator('#message')).toContainText(/confirm|email|X/i);
 ...
 await expect(page.locator('#msg')).toContainText(/sent|email|письмо/i);
