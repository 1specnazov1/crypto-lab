...
const ACCOUNT={effective_plan:'FREE',profile:{display_name:'Owner Smoke',language:'ru',timezone:'Europe/Kyiv',role:'admin'},subscription:{status:'active',current_period_end:null},limits:{daily_ai_requests:5,daily_backtests:50,daily_scanner_views:-1,daily_chart_views:100,daily_exact_backtests:25,max_portfolio_assets:-1,max_favorites:-1},usage_today:{ai_requests:0,backtests:0,scanner_views:0,chart_views:0,exact_backtests:0},counts:{portfolio_assets:0,favorites:0}};
...
(()=>{
 ...
 const plans=[{plan:'FREE',display_order:1,daily_ai_requests:5,daily_backtests:50,daily_scanner_views:-1,daily_chart_views:100,daily_exact_backtests:25,max_portfolio_assets:-1,max_favorites:-1}];
 ...
 const client={auth:{
   ...
   if(name==='get_crypto_feature_status')return{data:{allowed:true,remaining:5,limit:5},error:null};
...
async function stubExternalTraffic(page){
 await page.route('https://**/*',async route=>{
  ...
  if(url.includes('/functions/v1/crypto-lab-v79-register')) return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,enabled:true,registration_mode:'x_follower_free',follower_verification_required:true,target_handle:'@CryptoLabPulse',free_access:true,site_key:'turnstile-public-test',captcha_provider:'turnstile',captcha_action:'crypto_register',password_min_length:10,required_legal_keys:['terms','privacy','risk'],documents:[],readiness:{public_registration:true,turnstile:true,mail_provider:true,legal_documents:true}})});
...
test('public free account is English-first, hides paid plans and shows fair-use access',async({page})=>{await stubExternalTraffic(page);await page.goto('/v79/account.html?public-free-smoke=1',{waitUntil:'domcontentloaded'});await poll(page,()=>{const a=document.getElementById('accountView'),p=document.getElementById('plans'),b=document.getElementById('planBadge'),period=document.getElementById('periodEnd'),why=(document.getElementById('whyText')?.textContent||'').toLowerCase();if(!a||a.classList.contains('hide'))return'waiting';return document.documentElement.lang==='en'&&p?.style.display==='none'&&b?.textContent==='FREE'&&period?.textContent==='Free'&&why.includes('free for registered users')&&why.includes('@cryptolabpulse')?'free-ready':'waiting'},'free-ready','public free account')});
