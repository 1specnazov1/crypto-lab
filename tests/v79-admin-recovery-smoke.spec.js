...
async function stub(page){
  ...
  await page.route('https://txhzxbizjpinowepfjkm.supabase.co/functions/v1/crypto-lab-v79-recover',async route=>{
    ...
    if(route.request().method()==='GET'){
      return route.fulfill({status:200,contentType:'application/json',headers:CORS,body:JSON.stringify({ok:true,enabled:true,site_key:'test-site-key',captcha_action:'crypto_recover'})});
    }
