-- CRYPTO LAB v79
-- Authoritative candidate-only recovery after overlapping autonomous writers stopped.
-- The user proposed Trust Wallet but did not select a provider, network or asset.

-- Remove contradictory owner-authority triggers.
...
-- Remove all known false-authority constraints.
...
-- Preserve reconstructed records only as inactive audit history.
alter table public.crypto_owner_decision_records add column if not exists active boolean not null default true;
...
-- Restore candidate networks and remove invented provenance metadata.
...
set desired_mode='disabled',lifecycle_status='draft',checkout_enabled=false,webhook_enabled=false,
    ...
    last_verification=jsonb_build_object(
      ...
      'owner_network_decision_recorded',false,'asset_selected',false,'pricing_active',false,
      'receiving_addresses_active',false,'automatic_entitlement_enabled',false,'activation_allowed',false
    ),updated_at=now()
...
set status='decision_required',
    ...
    evidence=jsonb_build_object(
      ...
      'owner_approval_recorded',false,'invalid_owner_evidence_superseded',true,
      'overlapping_automation_disabled',true
    ),
    operator_note='Trust Wallet and direct on-chain payment are disabled candidates. The user did not select a provider rail, network, asset, price or receiving address. Future approval requires a new explicit user decision and a manual migration.',
...
set status='blocked_dependency',decision_summary='{}'::jsonb,
    evidence=jsonb_build_object(
      ...
      'invalid_owner_evidence_superseded',true,'overlapping_automation_disabled',true
    ),
...
-- Immutable helper predicates for validated CHECK constraints.
...
-- Trigger layer for explicit errors.
...
begin
  if new.approved_by_owner or new.status='active' or not private.crypto_onchain_candidate_metadata_safe(new.metadata) then
    raise exception 'On-chain owner approval is fail-closed until a future manual migration follows an explicit user decision' using errcode='42501';
  end if;
  return new;
end $$;
...
begin
  ...
  ) then raise exception 'Payment owner decision is fail-closed until a future manual migration follows an explicit user decision' using errcode='42501'; end if;
