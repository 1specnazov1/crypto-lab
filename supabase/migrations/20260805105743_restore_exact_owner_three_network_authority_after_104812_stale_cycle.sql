...
-- Restore the exact owner decision after stale concurrent migration 20260805104812.
...
-- No asset, price, address, sandbox transfer, checkout, recurring debit, refund or publication is enabled.
...
begin
  ...
  if v_sql is null then raise exception 'Canonical migration 20260805100805 not found'; end if;
  execute v_sql;
end $$;
...
set desired_mode='disabled',lifecycle_status='draft',checkout_enabled=false,webhook_enabled=false,
    ...
    last_verification=jsonb_build_object(
      'state','owner_approved_networks_inactive','decision_code','ONCHAIN_THREE_NETWORK_SELECTION',
      ...
      'owner_network_decision_recorded',true,'owner_provider_decision_recorded',false,
      'settlement_asset_selected',false,'pricing_active',false,'receiving_addresses_active',false,
      'automatic_entitlement_enabled',false,'activation_allowed',false,'concurrency_race_repaired',true,
      'superseded_stale_migration','20260805104812_authoritative_candidate_only_recovery_after_103945',
      'canonical_authority_migration','restore_exact_owner_three_network_authority_after_104812_stale_cycle'
    ),updated_at=now()
...
begin
  ...
  where network_code in('TRON','BSC','SOLANA')
    and not(approved_by_owner and status='inactive');
  if v_bad<>0 then raise exception 'Network state assertion failed: %',v_bad; end if;

  if (select count(*) from public.crypto_owner_decision_records
      where decision_code='ONCHAIN_THREE_NETWORK_SELECTION' and active)<>1 then
    raise exception 'Owner decision assertion failed';
  end if;

  if (select count(*) from public.crypto_owner_decision_authority_events
      where decision_code='ONCHAIN_THREE_NETWORK_SELECTION' and authority_state='effective')<>1 then
    raise exception 'Authority event assertion failed';
  end if;

  if exists(
    select 1 from public.crypto_billing_provider_adapters
    where provider='onchain'
      and(desired_mode<>'disabled' or lifecycle_status<>'draft' or checkout_enabled
          or webhook_enabled or recurring_enabled or refunds_enabled)
  ) then raise exception 'Adapter boundary assertion failed'; end if;
