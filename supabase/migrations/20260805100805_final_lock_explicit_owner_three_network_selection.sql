...
-- Canonical post-race owner decision lock.
-- Exact owner text: «Три сети утверждаю.»
-- UTF-8 SHA-256: 57458fbfe9da805c8dc8bec7ad2d8500516ca4568c35903565402aed62d848be
-- This approves TRON, BSC and SOLANA as the network set but does not activate payments.
...
drop function if exists private.crypto_owner_decision_record_delete_block();
...
drop function if exists private.crypto_owner_authority_event_immutable();
...
set effective=false,
    invalidated_by_migration='final_lock_explicit_owner_three_network_selection',
    invalidated_at=now(),
    invalidation_evidence=jsonb_build_object(
      'reason','Overlapping automation used stale context and contradicted the exact owner message.',
...
select
  ...
  jsonb_build_object(
    ...
    'approved_networks',jsonb_build_array('TRON','BSC','SOLANA'),
    'network_activation_authorized',false,
    'supersedes_migration','authoritative_candidate_only_payment_recovery',
...
where not exists(
  ...
  where decision_code='ONCHAIN_THREE_NETWORK_SELECTION'
    and event_type in('confirmed','corrected')
...
set approved_by_owner=true,
    ...
    metadata=jsonb_build_object(
      ...
      'canonical_authority_migration','final_lock_explicit_owner_three_network_selection'
    ),
    updated_at=now()
...
select private.service_update_crypto_launch_requirement(
  ...
  'Owner approved TRON, BSC and Solana by writing “Три сети утверждаю.” Networks remain inactive until remaining prerequisites and explicit activation approval are complete.'
);
...
select private.service_update_crypto_launch_requirement(
  ...
  'Approved three-network sandbox scope remains disabled pending remaining prerequisites.'
);
...
as $$ begin raise exception 'Owner decision records are immutable' using errcode='55000'; end $$;

create or replace function private.crypto_owner_authority_event_immutable()
returns trigger language plpgsql security definer set search_path='pg_catalog','pg_temp'
as $$ begin raise exception 'Owner decision authority events are append-only' using errcode='55000'; end $$;

create or replace function private.crypto_owner_decision_correction_supersession_immutable()
returns trigger language plpgsql security definer set search_path='pg_catalog','pg_temp'
as $$ begin raise exception 'Owner decision correction history is immutable' using errcode='55000'; end $$;

create trigger crypto_owner_decision_record_immutable before update or delete on public.crypto_owner_decision_records for each row execute function private.crypto_owner_decision_record_immutable();
create trigger crypto_owner_authority_event_immutable before update or delete on public.crypto_owner_decision_authority_events for each row execute function private.crypto_owner_authority_event_immutable();
...
as $$
declare v_hash constant text:='57458fbfe9da805c8dc8bec7ad2d8500516ca4568c35903565402aed62d848be';v_valid boolean;
begin
  select exists(
    select 1 from public.crypto_owner_decision_records r
    where r.decision_code='ONCHAIN_THREE_NETWORK_SELECTION'
      and r.decision_text='Три сети утверждаю.' and r.decision_hash=v_hash
      and r.source_channel='owner_chat' and r.active and not r.activation_authorized
      and exists(select 1 from public.crypto_owner_decision_authority_events e
        where e.decision_code=r.decision_code and e.event_type in('confirmed','corrected')
  ...
  if new.network_code in('TRON','BSC','SOLANA') then
    ...
    if new.metadata->>'decision_code'<>'ONCHAIN_THREE_NETWORK_SELECTION' or new.metadata->>'decision_hash'<>v_hash then
...
begin
  ...
  if new.code='PAYMENT_PROVIDER' and (
    ...
    or not coalesce((new.decision_summary->>'network_approval_recorded')::boolean,false)
  ) then raise exception 'Canonical payment owner decision mismatch' using errcode='42501'; end if;
  return new;
end $$;

create trigger crypto_onchain_network_owner_approval_guard before insert or update of approved_by_owner,status,metadata,network_code on public.crypto_onchain_networks for each row execute function private.enforce_crypto_onchain_owner_decision_record();
create trigger crypto_payment_owner_decision_provenance_guard before insert or update of status,decision_summary,evidence,operator_note,decided_at,verified_at on public.crypto_launch_requirements for each row execute function private.enforce_crypto_payment_owner_decision_record();

create or replace function private.crypto_owner_decision_integrity_snapshot()
returns jsonb language plpgsql stable security definer
set search_path='public','private','pg_catalog','pg_temp'
as $$
declare v_hash constant text:='57458fbfe9da805c8dc8bec7ad2d8500516ca4568c35903565402aed62d848be';v_checks jsonb;v_failures integer;
begin
  with checks(code,violations) as (
    select 'exact_active_immutable_decision_record',case when exists(select 1 from public.crypto_owner_decision_records where decision_code='ONCHAIN_THREE_NETWORK_SELECTION' and decision_text='Три сети утверждаю.' and decision_hash=v_hash and source_channel='owner_chat' and active and not activation_authorized) then 0 else 1 end
    union all select 'effective_authority_event',case when exists(select 1 from public.crypto_owner_decision_authority_events where decision_code='ONCHAIN_THREE_NETWORK_SELECTION' and event_type in('confirmed','corrected') and authority_state='effective' and decision_text_hash=v_hash) then 0 else 1 end
    union all select 'false_supersession_invalidated',case when not exists(select 1 from public.crypto_owner_decision_correction_supersessions where effective) then 0 else 1 end
    union all select 'three_networks_approved_inactive',case when (select count(*) from public.crypto_onchain_networks where network_code in('TRON','BSC','SOLANA') and approved_by_owner and status='inactive' and metadata->>'decision_code'='ONCHAIN_THREE_NETWORK_SELECTION' and metadata->>'decision_hash'=v_hash)=3 then 0 else 1 end
    union all select 'payment_requirement_matches_decision',case when exists(select 1 from public.crypto_launch_requirements where code='PAYMENT_PROVIDER' and status='in_progress' and decision_summary->>'decision_code'='ONCHAIN_THREE_NETWORK_SELECTION' and decision_summary->>'decision_hash'=v_hash and decision_summary->'approved_networks'=jsonb_build_array('TRON','BSC','SOLANA')) then 0 else 1 end
    union all select 'no_network_activation',case when (select count(*) from public.crypto_onchain_networks where status='active')=0 then 0 else 1 end
    union all select 'payment_adapter_disabled',case when exists(select 1 from public.crypto_billing_provider_adapters where provider='onchain' and desired_mode='disabled' and lifecycle_status='draft' and not checkout_enabled and not webhook_enabled and not recurring_enabled and not refunds_enabled) then 0 else 1 end
    union all select 'canonical_constraints_present',case when (select count(*) from pg_constraint where conname in('crypto_owner_decision_canonical_three_network_check','crypto_onchain_canonical_three_network_approval_check','crypto_payment_provider_owner_decision_check'))=3 then 0 else 1 end
  )
  select jsonb_agg(jsonb_build_object('code',code,'violations',violations,'passed',violations=0) order by code),sum(case when violations>0 then 1 else 0 end) into v_checks,v_failures from checks;
...
comment on table public.crypto_owner_decision_authority_events is 'Append-only authority events resolving conflicting autonomous evidence.';
