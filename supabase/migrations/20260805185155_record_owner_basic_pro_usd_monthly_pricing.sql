...
-- Record owner-approved USD monthly pricing while preserving explicit payment activation denial.

do $$
declare
  v_text constant text := E'BASIC — 20 грн/месяц;\nPRO — 40 грн/месяц - внеси изменения для крипто лаб пиши в долларах. И напиши для мен ниже кратко по пунктам что включают в себя эти тарифы';
  v_hash constant text := 'db8ef6f56587b0aa05c602c838dc94acc06d6dca5a42d6f2e76130a7c7e198c0';
begin
  if encode(extensions.digest(convert_to(v_text,'UTF8'),'sha256'),'hex') <> v_hash then
    raise exception 'Pricing owner decision hash mismatch';
  end if;
  ...
  insert into public.crypto_owner_decision_records(
    ...
    'ONCHAIN_BASIC_PRO_USD_MONTHLY_PRICING',v_text,v_hash,'owner_chat',
    jsonb_build_object(
      'decision_scope','pricing_selection_only',
      'currency','USD','billing_interval','month',
      'basic_amount_minor',2000,'pro_amount_minor',4000,
      'display_basic','$20/month','display_pro','$40/month',
      'selected_assets',jsonb_build_array('USDT','USDC'),
      'payment_activation_authorized',false,
      'pricing_activation_authorized',false,
      'replaces_pricing_proposal',jsonb_build_object('currency','UAH','basic_amount_minor',39900,'pro_amount_minor',79900)
    ),false,now(),true
  ) on conflict(decision_code) do nothing;

  if not exists(
    select 1 from public.crypto_owner_decision_records
    where decision_code='ONCHAIN_BASIC_PRO_USD_MONTHLY_PRICING'
      and decision_hash=v_hash and source_channel='owner_chat' and active and not activation_authorized
  ) then raise exception 'Pricing owner decision record was not created'; end if;
  ...
  insert into public.crypto_owner_decision_authority_events(
    ...
    'ONCHAIN_BASIC_PRO_USD_MONTHLY_PRICING','confirmed','effective',v_hash,
    jsonb_build_object(
      'exact_owner_text',v_text,'currency','USD','billing_interval','month',
      'basic_amount_minor',2000,'pro_amount_minor',4000,
      'payment_activation_authorized',false,'pricing_activation_authorized',false
    ),now()
  where not exists(
...
delete from public.crypto_plan_prices
where plan in('BASIC','PRO') and currency='UAH' and billing_interval='month';

insert into public.crypto_plan_prices(plan,currency,billing_interval,amount_minor,provider,active)
values
  ('BASIC','USD','month',2000,'onchain_candidate',false),
  ('PRO','USD','month',4000,'onchain_candidate',false)
...
set accounting_currency='USD',
    accounting_amount_minor=case plan when 'BASIC' then 2000 when 'PRO' then 4000 end,
    ...
    metadata=jsonb_build_object(
      ...
      'owner_approved',true,
      'decision_code','ONCHAIN_BASIC_PRO_USD_MONTHLY_PRICING',
      'decision_hash','db8ef6f56587b0aa05c602c838dc94acc06d6dca5a42d6f2e76130a7c7e198c0',
      'currency','USD','billing_interval','month',
      'amount_usd',case plan when 'BASIC' then 20 when 'PRO' then 40 end,
...
alter table public.crypto_plan_prices add constraint crypto_paid_plan_owner_usd_pricing_check check(
  plan='FREE' or (
    ...
    and ((plan='BASIC' and amount_minor=2000) or (plan='PRO' and amount_minor=4000))
  )
);
...
alter table public.crypto_onchain_plan_pricing add constraint crypto_onchain_owner_usd_pricing_check check(
  plan not in('BASIC','PRO') or asset_code not in('USDT','USDC') or (
    ...
    and ((plan='BASIC' and accounting_amount_minor=2000) or (plan='PRO' and accounting_amount_minor=4000))
...
set status='verified',
    ...
    decision_summary=jsonb_build_object(
      ...
      'currency','USD','billing_interval','month',
      'basic_amount_minor',2000,'pro_amount_minor',4000,
      'owner_approved',true,'activation_authorized',false
    ),
    evidence=jsonb_build_object(
      'selected_assets',jsonb_build_array('USDT','USDC'),
      'basic_display','$20/month','pro_display','$40/month',
      'onchain_price_rows',4,'active_onchain_price_rows',0,
      'single_paid_currency',true,'obsolete_uah_rows_removed',true
    ),
    operator_note='Owner approved BASIC at $20/month and PRO at $40/month. Pricing is recorded but remains inactive because payment activation is not authorized.',
    decided_at=now(),verified_at=now(),updated_at=now()
where code='PRICING_MODEL';

update public.crypto_launch_requirements
set evidence=evidence || jsonb_build_object(
      'pricing_decision_code','ONCHAIN_BASIC_PRO_USD_MONTHLY_PRICING',
      'pricing_decision_hash','db8ef6f56587b0aa05c602c838dc94acc06d6dca5a42d6f2e76130a7c7e198c0',
      'pricing_currency','USD','basic_amount_minor',2000,'pro_amount_minor',4000,
      'pricing_owner_approved',true,'active_price_count',0
    ),
    operator_note='Networks, assets and USD monthly pricing are owner-approved. All payment execution remains disabled until a separate explicit activation decision.',
    updated_at=now()
...
begin
  if (select count(*) from public.crypto_plan_prices where plan in('BASIC','PRO'))<>2 then
    raise exception 'Paid plan price row count mismatch';
  end if;
  if exists(select 1 from public.crypto_plan_prices where plan='BASIC' and (currency<>'USD' or amount_minor<>2000 or active))
     or exists(select 1 from public.crypto_plan_prices where plan='PRO' and (currency<>'USD' or amount_minor<>4000 or active)) then
    raise exception 'Paid plan USD pricing assertion failed';
  end if;
  if (select count(*) from public.crypto_onchain_plan_pricing where plan in('BASIC','PRO') and asset_code in('USDT','USDC')
      and accounting_currency='USD' and not active and approved_at is not null)<>4 then
    raise exception 'On-chain USD pricing assertion failed';
  end if;
