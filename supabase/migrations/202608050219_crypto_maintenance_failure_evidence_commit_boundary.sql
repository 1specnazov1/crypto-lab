drop function if exists private.cron_run_crypto_lab_daily_maintenance();

create or replace procedure private.cron_run_crypto_lab_daily_maintenance()
language plpgsql
security invoker
as $$
declare
...
begin
  ...
  if session_user<>'postgres' or current_user<>'postgres' then
    raise exception 'CRYPTO LAB daily maintenance procedure requires postgres cron owner'
      using errcode='42501';
  end if;
...
revoke all on procedure private.cron_run_crypto_lab_daily_maintenance() from public,anon,authenticated,service_role;
grant execute on procedure private.cron_run_crypto_lab_daily_maintenance() to postgres;
...
as $$
declare
  v_expected_command constant text := 'call private.cron_run_crypto_lab_daily_maintenance();';
...
select cron.alter_job(
  5,
  command := 'call private.cron_run_crypto_lab_daily_maintenance();'
);