...
Deno.serve(async req=>{
  ...
  if(action==="redeem"){
    ...
    if(grant.revoked_at||grant.used_at||grant.redeem_started_at||new Date(grant.expires_at).getTime()<=Date.now())return json({ok:false,error:"Ссылка уже используется, использована или истекла",code:"LINK_EXPIRED"},410,origin);
    ...
    const {data:reserved,error:reserveErr}=await admin.from("crypto_email_access_grants").update({redeem_started_at:reservedAt,updated_at:reservedAt}).eq("id",grant.id).is("redeem_started_at",null).is("used_at",null).is("revoked_at",null).gt("expires_at",reservedAt).select("id").maybeSingle();
    if(reserveErr)throw reserveErr;if(!reserved)return json({ok:false,error:"Ссылка уже используется или использована. Запросите новую.",code:"LINK_IN_PROGRESS"},409,origin);
    try{
      ...
      const {error:updateErr}=await admin.from("crypto_email_access_grants").update({user_id:userId,updated_at:new Date().toISOString()}).eq("id",grant.id).eq("redeem_started_at",reservedAt);if(updateErr)throw updateErr;
      return json({ok:true,status:"verified_device",grant_id:grant.id,token_hash:hashed,email_masked:maskEmail(grant.email)},200,origin);
    }catch(error){await admin.from("crypto_email_access_grants").update({redeem_started_at:null,user_id:null,updated_at:new Date().toISOString()}).eq("id",grant.id).eq("redeem_started_at",reservedAt).is("used_at",null);throw error}
  }
  ...
  if(action==="claim"){
    ...
    const {data:claimed,error:claimErr}=await admin.from("crypto_email_access_grants").update({used_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",grant.id).is("used_at",null).select("id").maybeSingle();if(claimErr)throw claimErr;if(!claimed)return json({ok:false,error:"Ссылка уже использована",code:"LINK_EXPIRED"},410,origin);
    return json({ok:true,status:"activated",user_id:user.id,email:user.email},200,origin);
  }
