...
(() => {
  ...
  const labels = {
    ru:{title:'Операционная готовность',refresh:'Обновить здоровье',scanner:'Сканер',monitor:'Монитор',stale:'Зависшие запуски',maintenance:'Обслуживание',healthy:'Норма',warning:'Внимание',unknown:'Нет данных',age:'мин назад',signals:'Сигналы',ai:'AI',backtest:'Бэктест',cron:'Фоновые задания',name:'Задание',schedule:'Расписание',status:'Статус',last:'Последний запуск',message:'Результат',registration:'Регистрация за 24 часа',deletions:'Запросы на удаление',none:'Нет данных',email:'Email',reason:'Причина',requested:'Запрошено',loading:'Загрузка операционной диагностики…'},
    uk:{title:'Операційна готовність',refresh:'Оновити стан',scanner:'Сканер',monitor:'Монітор',stale:'Завислі запуски',maintenance:'Обслуговування',healthy:'Норма',warning:'Увага',unknown:'Немає даних',age:'хв тому',signals:'Сигнали',ai:'AI',backtest:'Бектест',cron:'Фонові завдання',name:'Завдання',schedule:'Розклад',status:'Статус',last:'Останній запуск',message:'Результат',registration:'Реєстрація за 24 години',deletions:'Запити на видалення',none:'Немає даних',email:'Email',reason:'Причина',requested:'Запитано',loading:'Завантаження операційної діагностики…'},
    en:{title:'Operational readiness',refresh:'Refresh health',scanner:'Scanner',monitor:'Monitor',stale:'Stale runs',maintenance:'Maintenance',healthy:'Healthy',warning:'Attention',unknown:'No data',age:'min ago',signals:'Signals',ai:'AI',backtest:'Backtest',cron:'Background jobs',name:'Job',schedule:'Schedule',status:'Status',last:'Last run',message:'Result',registration:'Registration in 24 hours',deletions:'Deletion requests',none:'No data',email:'Email',reason:'Reason',requested:'Requested',loading:'Loading operational diagnostics…'}
  };
  ...
  function listObject(value){
    ...
    return entries.length?entries.map(([key,count])=>`<div class="request-head"><span>${esc(key)}</span><b>${number(count)}</b></div>`).join(''):`<div class="health-muted">${t.none}</div>`;
  }
  function table(headers,rows){
    if(!rows.length)return `<div class="health-muted">${t.none}</div>`;
    return `<div class="health-table"><table><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${rows.join('')}</tbody></table></div>`;
  }

  function render(data){
    const scanner=data?.scanner||{},monitor=data?.monitor||{},stale=data?.stale_runs||{},maintenance=data?.last_maintenance||{},jobs=Array.isArray(data?.cron_jobs)?data.cron_jobs:[],deletions=Array.isArray(data?.pending_deletions)?data.pending_deletions:[];
  ...
  async function load(){
    ...
    ensure();busy=true;$('healthRefresh').disabled=true;$('healthBody').innerHTML=`<div class="health-muted">${t.loading}</div>`;
    try{const {data,error}=await sb.rpc('get_crypto_admin_operational_health');if(error)throw error;render(data||{});}
    catch(error){$('healthBody').innerHTML=`<div class="health-error">${esc(error?.message||error)}</div>`;}
    finally{busy=false;$('healthRefresh').disabled=false;}
  }

  function boot(){
    const panel=ensure();if(!panel)return;
    const dashboard=$('dashboard'),nowVisible=!!dashboard&&!dashboard.classList.contains('hide');
    if(nowVisible&&!visible)load();
    visible=nowVisible;
    const refresh=$('refresh');
    if(refresh&&!refresh.dataset.healthHook){refresh.dataset.healthHook='1';refresh.addEventListener('click',()=>setTimeout(load,0));}
  }
