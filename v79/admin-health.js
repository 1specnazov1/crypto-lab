...
(() => {
  ...
  function notify(text,bad=false){if(typeof message==='function')message(text,bad);else window.alert(text);}
  function setActionBusy(value){actionBusy=value;document.querySelectorAll('[data-outbox-requeue]').forEach(button=>button.disabled=value);if($('healthRefresh'))$('healthRefresh').disabled=value||busy;}

  async function requeue(id){
    if(actionBusy||busy||!id)return;
    const entered=window.prompt(t.requeuePrompt);
    if(entered===null)return;
    const reason=String(entered).replace(/[\u0000-\u001f\u007f]+/g,' ').trim();
    if(reason.length<5||reason.length>500){notify(t.reasonRequired,true);return;}
    setActionBusy(true);
    let reload=false;
    try{
      const {data,error}=await sb.rpc('admin_requeue_crypto_signal_notification',{p_id:id,p_reason:reason});
      if(error)throw error;
      if(data?.requeued!==true)throw new Error(t.requeueFailed);
      notify(t.requeueSuccess);
      reload=true;
    }catch(error){notify(error?.message||t.requeueFailed,true);}finally{setActionBusy(false);}
    if(reload)await load();
  }

  function bindOutboxActions(){document.querySelectorAll('[data-outbox-requeue]').forEach(button=>{button.onclick=()=>requeue(button.dataset.outboxRequeue);});}
  ...
  function render(data){
    ...
    $('healthBody').innerHTML=`
      ...
      <div class="health-panel" style="margin-top:10px"><h3>${t.outbox} · ${number(outboxRecent.length)}</h3>${table([t.event,t.asset,t.status,t.attempts,t.telegram,t.created,t.error,t.action],outboxRecent.map(item=>{const canRequeue=item.status==='dead'||item.status==='retry';return `<tr><td><b>${esc(item.event_type||'—')}</b></td><td>${esc(item.symbol||'—')} · ${esc(item.timeframe||'—')} · ${esc(item.direction||'—')}</td><td>${statusBadge(item.status)}</td><td>${number(item.attempts)}</td><td>${esc(item.telegram_message_id||'—')}</td><td>${when(item.created_at)}</td><td><span class="outbox-error">${esc(item.last_error||'—')}</span></td><td>${canRequeue?`<button class="gold outbox-requeue" data-outbox-requeue="${esc(item.id)}">${t.requeue}</button>`:'—'}</td></tr>`;}))}</div>
