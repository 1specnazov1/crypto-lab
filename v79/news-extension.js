...
(() => {
  ...
  if(!document.getElementById('newsTickerStyles')){const s=document.createElement('style');s.id='newsTickerStyles';s.textContent=`
    .main{grid-template-rows:60px auto minmax(0,1fr)!important}
    .main>.top{grid-row:1}.main>.market-news-ticker{grid-row:2}.main>.work{grid-row:3;min-height:0!important}
    .market-news-ticker{display:none;position:relative;z-index:4;height:31px;min-height:31px;overflow:hidden;border-bottom:1px solid #f6465d55;background:linear-gradient(90deg,#241216,#171417 42%,#19170e);cursor:pointer;white-space:nowrap}
    .market-news-ticker.show{display:block}.market-news-ticker:before{content:attr(data-label);position:absolute;left:0;top:0;bottom:0;z-index:2;display:flex;align-items:center;padding:0 10px;background:#2b1217;color:#ff8a9b;font-size:10px;font-weight:950;letter-spacing:.06em;border-right:1px solid #f6465d55}
    ...
    @media(max-width:680px){.main{grid-template-rows:56px auto minmax(0,1fr)!important}.market-news-ticker{height:29px;min-height:29px}.market-news-ticker:before{padding:0 7px;font-size:9px}.market-news-track{padding-left:92px;font-size:10px;animation-duration:24s}}
  ...
  function region(v){const key=String(v||'GLOBAL').toUpperCase();const map={ru:{US:'США',EUROPE:'ЕВРОПА',CHINA:'КИТАЙ',JAPAN:'ЯПОНИЯ',OTHER:'ДРУГИЕ',GLOBAL:'МИР'},uk:{US:'США',EUROPE:'ЄВРОПА',CHINA:'КИТАЙ',JAPAN:'ЯПОНІЯ',OTHER:'ІНШІ',GLOBAL:'СВІТ'},en:{US:'US',EUROPE:'EUROPE',CHINA:'CHINA',JAPAN:'JAPAN',OTHER:'OTHER',GLOBAL:'GLOBAL'}};return(map[lang]||map.ru)[key]||key}
  function impact(){return lang==='uk'?'Вплив':lang==='en'?'Impact':'Влияние'}
  function breakingLabel(){return lang==='uk'?'⚡ ТЕРМІНОВО':lang==='en'?'⚡ BREAKING':'⚡ СРОЧНО'}
  function syncTickerLabel(){if(ticker)ticker.dataset.label=breakingLabel()}
  async function refreshTicker(){syncTickerLabel();const tk=authToken();if(!tk){ticker?.classList.remove('show');return}try{const r=await fetch(ENDPOINT+'?limit=30&min_impact=75&lang='+encodeURIComponent(lang),{headers:{Authorization:`Bearer ${tk}`,apikey:APIKEY},cache:'no-store'});const j=await r.json();if(!r.ok||!j?.ok)throw Error();const arr=(j.breaking||[]).slice(0,5);if(!arr.length){ticker?.classList.remove('show');window.CRYPTO_NEWS_TICKER_STATUS={ok:true,breaking:0,last_refresh:new Date().toISOString(),lang};return}const text=arr.map(x=>`<b>${esc(fmt(x.published_at))} · ${esc(region(x.region))}</b> · ${esc(x.localized_title||x.title)} · <span class="src">${esc(x?.metadata?.source_name||x.domain||'')} · ${impact()} ${esc(x.impact_score)} · ${esc(dir(x.direction))}</span>`).join('&nbsp;&nbsp;&nbsp; ◆ &nbsp;&nbsp;&nbsp;');const track=document.getElementById('marketNewsTrack');if(track){track.innerHTML=text;track.style.animationDuration=Math.max(24,Math.min(75,text.replace(/<[^>]+>/g,'').length/8))+'s'}ticker?.classList.add('show');window.CRYPTO_NEWS_TICKER_STATUS={ok:true,breaking:arr.length,last_refresh:new Date().toISOString(),lang}}catch{ticker?.classList.remove('show');window.CRYPTO_NEWS_TICKER_STATUS={ok:false,breaking:0,last_refresh:new Date().toISOString(),lang}}}
  const baseTranslate=translate;translate=function newsTranslate(){baseTranslate();enhanceNewsFrame();syncTickerLabel();refreshTicker()};translate();
