...
(() => {
  const labels = {
    ru: { title:'Последние серверные бэктесты', note:'Хранятся только параметры и итоговая статистика, без сырых свечей', refresh:'Обновить', date:'Дата', status:'Статус', market:'Рынок', trades:'Сделок', net:'P&L', ret:'Доходность', duration:'Время', error:'Ошибка', empty:'История пока пуста', auth:'Требуется вход в аккаунт' },
    uk: { title:'Останні серверні бектести', note:'Зберігаються лише параметри та підсумкова статистика, без сирих свічок', refresh:'Оновити', date:'Дата', status:'Статус', market:'Ринок', trades:'Угод', net:'P&L', ret:'Дохідність', duration:'Час', error:'Помилка', empty:'Історія поки порожня', auth:'Потрібен вхід в акаунт' },
    en: { title:'Recent server backtests', note:'Only parameters and summary statistics are stored; raw candles are not retained', refresh:'Refresh', date:'Date', status:'Status', market:'Market', trades:'Trades', net:'P&L', ret:'Return', duration:'Time', error:'Error', empty:'No history yet', auth:'Sign in is required' }
  };
  ...
  function applyLabels() {
    mount();
    const t = text();
    const map = { historyTitle:'title', historyNote:'note', historyRefresh:'refresh', historyDate:'date', historyStatus:'status', historyMarket:'market', historyTrades:'trades', historyNet:'net', historyReturn:'ret', historyDuration:'duration', historyError:'error' };
