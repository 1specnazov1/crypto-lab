...
(() => {
  function injectAnalyticsTools(){
    ...
    if(!doc.getElementById('cryptoChartLivePriceScript')){
      const live=doc.createElement('script');live.id='cryptoChartLivePriceScript';live.src='./chart-live-price.js?v=7930free23';doc.body.appendChild(live);
    }
        ...
        scale.onload=()=>{
          ...
          hook.onload=()=>{
            ...
            onchain.onload=()=>{
              ...
              const smart=doc.createElement('script');smart.id='cryptoSmartMoneyScript';smart.src='./chart-smart-money.js?v=7930smart1';doc.body.appendChild(smart);
            };
  ...
  function injectNewsUi(){
    const frame=document.getElementById('frame');if(!frame)return;let doc,path='';
    ...
    const script=doc.createElement('script');script.id='cryptoNewsUiUpgradesScript';script.src='./news-ui-upgrades.js?v=7930free26';doc.body.appendChild(script);
  }
  function injectAiSmartMoney(){
    const frame=document.getElementById('frame');if(!frame)return;let doc,path='';
    try{doc=frame.contentDocument;path=frame.contentWindow?.location?.pathname||''}catch{return}
    if(!doc||!path.endsWith('/ai.html')||doc.getElementById('cryptoAiSmartMoneyScript'))return;
    const script=doc.createElement('script');script.id='cryptoAiSmartMoneyScript';script.src='./ai-smart-money.js?v=7930smart1';doc.body.appendChild(script);
  }
  ...
  function loadNewsExtension(){
    ...
    const script=document.createElement('script');script.id='cryptoNewsExtensionScript';script.src='./news-extension.js?v=7930free22';
    script.onload=()=>{if(document.getElementById('cryptoNewsTickerControlsScript'))return;const controls=document.createElement('script');controls.id='cryptoNewsTickerControlsScript';controls.src='./news-ticker-controls.js?v=7930free26';document.body.appendChild(controls)};
  ...
  try {
    ...
    document.getElementById('frame')?.addEventListener('load',()=>setTimeout(enhanceFrame,0));translate();loadNewsExtension();
