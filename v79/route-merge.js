...
(() => {
  ...
  function injectNewsUi(){const frame=document.getElementById('frame');if(!frame)return;let doc,path='';try{doc=frame.contentDocument;path=frame.contentWindow?.location?.pathname||''}catch{return}if(!doc||!path.endsWith('/news.html')||doc.getElementById('cryptoNewsUiUpgradesScript'))return;const script=doc.createElement('script');script.id='cryptoNewsUiUpgradesScript';script.src='./news-ui-upgrades.js?v=7930free26';doc.body.appendChild(script)}
  function injectAiSmartMoney(){const frame=document.getElementById('frame');if(!frame)return;let doc,path='';try{doc=frame.contentDocument;path=frame.contentWindow?.location?.pathname||''}catch{return}if(!doc||!path.endsWith('/ai.html')||doc.getElementById('cryptoAiSmartMoneyScript'))return;const script=doc.createElement('script');script.id='cryptoAiSmartMoneyScript';script.src='./ai-smart-money.js?v=7930smart1';doc.body.appendChild(script)}
